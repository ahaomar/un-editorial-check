...
Keep `package.json`, `VERSION` and the newest `CHANGELOG.md` heading identical. Keep version `0.6.0` unless the documented release process requires a version change. The package has no dependencies, so no lockfile is required.
