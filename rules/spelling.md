...
- **UE-SP003** *(info, opt-in)* — Report `-ize` forms that may require an `-ise` spelling under the named dictionary. Off unless `spellingReview` is enabled in configuration, because Oxford spelling is legitimate and the UN framework does not settle every case.
